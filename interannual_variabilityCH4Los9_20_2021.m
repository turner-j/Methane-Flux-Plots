...
yr = year(USLos.TIMESTAMP_END(daily_WTD(:,2)*48));
...
nanmean(dailyTairf(year(CH4dates)==2020))
%% Heuristic Model
...
subplot(2,2,3)
Y = round(100*dailyWTD((yr==2019)&~isnan(dailyWTD)));
Y = Y - min(Y)+1;
Fn19 = accumarray(Y,Fn((yr==2019)&(~isnan(dailyWTD))),[],@nanmean);
% make sure not to include NaNs in polyfit
plot(unique(Y)/100,Fn19(unique(Y)),'o','MarkerFaceColor',[0.3020 0.7451 0.9333],...
...
% make sure not to include NaNs in polyfit
hold on
Y = round(100*dailyWTD((yr==2020)));
Y = Y - min(Y)+1;
Y = Y(~isnan(dailyCH4(yr==2020)));
Fn20 = Fn(year(modeldates)==2020);
Fn20 = accumarray(Y(~isnan(Y)),Fn20(~isnan(Y)),[],@nanmean);
plot(unique(Y(~isnan(Y)))/100,Fn20(unique(Y(~isnan(Y)))),'o','MarkerFaceColor',[0.3020 0.7451 0.9333],...
    'MarkerEdgeColor',[0.3020 0.7451 0.9333])
...
for k = 1:90
    B = NaN(length(Fn19),1);
    B((k+1):end) = Fn19(1:(end-k));% SHIFT CH4 DOWN
    Z(:,k) = B;
end
...
for i = 1:90
    GPP = A(:,i);
    TaCH4 = Z(:,i);
    [rhob(i),pvalb(i)] = corr(GPP(~isnan(GPP)),TaCH4(~isnan(TaCH4)));
end
%% Repeat Lag Analysis with just GPP and FCH4
Z = [];
dailyGPP19 = dailyGPP(~isnan(dailyTairf)&~isnan(dailyCH4));
...
for k = 1:90
    B = NaN(length(dailyCH419),1);
    B((k+1):end) = dailyCH419(1:(end-k));% shift GPP down
    Z(:,k) = B;
end
...
for i = 1:90
    GPP = A(:,i);
    CH4 = Z(:,i);
    [rhoc(i),pvalc(i)] = corr(GPP(~isnan(GPP)),CH4(~isnan(CH4)));
end
%% Rename variables
lag = 0:-1:-89;
% GPP and Ta-CH4
w3 = lag(pvalb<0.05 & rhob>0);
v3 = rhob(pvalb<0.05 & rhob>0);
% GPP and FCH4
Los2019lagb = lag(pvalc<0.05 & rhoc>0);
Los2019lagcorrb = rhoc(pvalc<0.05 & rhoc>0);
%% Repeat for 2020
Z = [];
...
subplot(1,3,1)
hold on
plot(Los2019lagb',Los2019lagcorrb','--','Color',[0.3020 0.7451 0.9333],'LineWidth',2)% original
plot(w3',v3','-','Color',[0.0980    0.3176    0.4118],'LineWidth',2)% normalized
hold off
