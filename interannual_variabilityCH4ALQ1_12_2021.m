...
% 
% subplot(1,2,1)
% plot(dis.discharge(yr2==2019),dailyCH4(yr2==2019),'o','MarkerEdgeColor',[ 0.4902    0.2588    0.4000])% Linear fit
% xlabel('Discharge (^{m^3}/_{s})')
% ylabel('T_{air} FCH_4')
% 
% subplot(1,2,2)
% plot(dis.discharge(yr2==2020),dailyCH4(yr2==2020),'o','MarkerEdgeColor',[0.8784    0.4588    0.7176])
...
subplot(1,2,1)
hold on
x = dis.Twater_mean(~isnan(dailyCH4)&~isnan(dailyTairf)&~isnan(dis.Twater_mean)&(ismember(month(dis.datetime),4:10)));
y = Fn(~isnan(dailyCH4)&~isnan(dailyTairf)&~isnan(dis.Twater_mean)&(ismember(month(dis.datetime),4:10)));
